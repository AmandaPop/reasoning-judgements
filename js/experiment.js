...
function startExperiment() {
  ...
  const verb_condition = 'believe' //jsPsych.randomization.randomInt(0, 1) === 0 ? 'think' : 'believe'; //just edit manually so only one verb at a time
...
const consent = {
  ...
  const response_template = {
    ...
    labels: function() {
      return [
        `<div>${jsPsych.timelineVariable('no')}</div>`,
        `<div>${jsPsych.timelineVariable('yes')}</div>`
      ];
    },
    slider_width: 500,
